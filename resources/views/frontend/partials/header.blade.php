        ...
        <div class="collapse navbar-collapse" id="navbarMenu">
          <ul class="navbar-nav mx-auto">
            @foreach($info['cates'] as $key => $item)
              @if(empty($item['subChild']))
                <li class="nav-item">
                  <a href="#" class="nav-link">{{ $item['name_category'] }}</a>
                </li>
              @else
                <li class="nav-item dropdown">
                  <a class="nav-link dropdown-toggle" href="category.html" id="dropdown-{{ $item['id'] }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{ $item['name_category'] }}</a>
                  <div class="dropdown-menu">
                    @foreach($item['subChild'] as $k =>$val)
                      <a class="dropdown-item" href="#">{{ $val['name_category'] }}</a>
                    @endforeach
                  </div>
                </li>
              @endif  
